...
def find_flights(flight_time):
    ...
    for flight in flight_details:
        if int(flight.split(":")[3])>=flight_time and int(flight.split(":")[3])<=flight_time+200:
            temp.append(flight)
    return temp
...
def sort_flight_list(flight_list):
    #Remove pass and write your logic here
    flight_list.sort(key=lambda x: int(x.split(":")[3]))
    return flight_list
...
def get_passenger_details(flight_detail):
    ...
    for key,value in passenger_details_dict.items():
        if flight_detail.split(":")[0] == value[1]:
            temp.append(key)
    return (temp)
...
def security_check(passenger_pnr_list):
    ...
    for pnr in passenger_pnr_list:
        value=passenger_details_dict[pnr]
        if value[3] in range(0,26):
            temp.append(pnr)
    return temp
